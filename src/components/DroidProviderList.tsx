  ...
  (
    ...
    const fetchBalance = async (provider: DroidProvider) => {
      ...
      try {
        ...
        console.log(
          "data.usage 的所有字段:",
          data.usage ? Object.keys(data.usage) : "null",
        );
        ...
        setBalances((prev) => {
          ...
          console.log(
            "newBalances[provider.id].end_date:",
            newBalances[provider.id].end_date,
          );
          return newBalances;
        });
    ...
    useEffect(() => {
      ...
      if (!currentProvider || !currentProvider.api_key) {
        return;
      }

      // 获取刷新间隔（分钟），默认 60 分钟
      const refreshInterval = currentProvider.refresh_interval || 60;
      const intervalMs = refreshInterval * 60 * 1000;

      console.log(
        `Setting up auto-refresh for ${currentProvider.name} with interval: ${refreshInterval} minutes`,
      );
      ...
      const intervalId = setInterval(() => {
        // 使用 ref 获取最新的 providers
        const provider = providersRef.current.find(
          (p) => p.id === currentProviderId,
        );
        if (provider && provider.api_key) {
          const interval = provider.refresh_interval || 60;
          console.log(
            `Auto-refreshing balance for ${provider.name} (interval: ${interval} min)`,
          );
          fetchBalance(provider);
        }
      }, intervalMs);

      // 清理函数：组件卸载或 currentProviderId/refreshInterval 变化时清除定时器
      return () => {
        console.log(
          `Clearing balance refresh timer for provider ${currentProviderId}`,
        );
        clearInterval(intervalId);
      };
    }, [
      currentProviderId,
      providers.find((p) => p.id === currentProviderId)?.refresh_interval,
    ]);
                    ...
                    {balance?.end_date &&
                      (() => {
                        ...
                        const daysLeft = Math.ceil(
                          (balance.end_date - now) / (1000 * 60 * 60 * 24),
                        );
                        const colorClass =
                          daysLeft < 7
                            ? "text-red-600 dark:text-red-400"
                        ...
                        return (
                          ...
                          >
                            到期:{" "}
                            {new Date(balance.end_date).toLocaleDateString(
                              "zh-CN",
                    ...
                    <div className="flex items-center justify-between gap-2 text-gray-600 dark:text-gray-400">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-mono text-xs text-gray-500 dark:text-gray-400">
                          {provider.api_key.slice(0, 10)}...
                          {provider.api_key.slice(-6)}
                        </span>
                      </div>
                        ...
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-3">
                            <div className="text-xs text-gray-500 dark:text-gray-400">
                              ...
                              总额{" "}
                              {(balance.totalAllowance / 1000000).toFixed(1)}M
                            </div>
                            <div
                              className={cn(
                                "text-sm font-medium",
                      ...
                      <div className="mt-2">
                        <div className="h-2 bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, balance.usedRatio * 100)}%`,
                            }}
                          />
                        </div>
                      </div>
